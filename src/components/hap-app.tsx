...
export function HapApp({ path, publicContext = false, slug, title = "Hap" }: HapAppProps) {
  ...
  // Receive navigation performed inside the app, plus its readiness handshake.
  // The handshake matters for legacy URLs: the app resolves them to their
  // canonical path and reports it back even if this listener mounted late.
  useEffect(() => {
    if (publicContext) return;
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type !== "hap:navigate" && data.type !== "hap:ready") return;
      const next = data.path;
      if (next === pathname) return;
      lastSent.current = next;
      void router.navigate({ to: next, replace: data.type === "hap:ready" });
    };
